...
static void InstallPixmap ( Widget w )
{
   ...
   /*
    * Create a bitmap of the X logo, and then an empty
    * pixmap of the same size.
    */

    bitmap = 
        XCreateBitmapFromData ( XtDisplay ( w ), 
                                RootWindowOfScreen ( XtScreen ( w ) ),
                                xlogo64_bits, xlogo64_width, 
                                xlogo64_height );
    xlogoPixmap = 
              XCreatePixmap ( XtDisplay ( w ), 
                              RootWindowOfScreen ( XtScreen ( w ) ),
                              xlogo64_width, xlogo64_height, depth );
   /*
    * Copy the contents of plane 1 of the bitmap to the 
    * pixmap, using the widget's foreground and background colors.
    */

    XCopyPlane ( XtDisplay ( w ), bitmap, xlogoPixmap, gc, 0, 0, 
                                 ...
                                 XtPointer callData )
{
    ...
    */
    ...
    XtVaGetValues ( w,  
                   ...
                   NULL  );
  /*
   * Copy the pixmap to the center of the window
   */

   XCopyArea ( XtDisplay ( w ), xlogoPixmap, XtWindow ( w ), 
               gc, 0, 0, xlogo64_width, xlogo64_height, 
               ( width - xlogo64_width ) / 2,  
               ( height - xlogo64_height ) / 2 );
}
