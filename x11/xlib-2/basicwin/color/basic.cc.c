...
char **argv;
{
	...
	width = display_width/3, height = display_height/4;
		...
		* Converting between these two is painful.
		...
		best_map_info.colormap = XCreateColormap(display, RootWindow(display, screen_num), visual, AllocAll);

		if (best_map_info.colormap == DefaultColormap(display, screen_num)) {
		...
		for (i = 0; i < best_map_info.blue_max; i++) {
			...
			for (j = 0; j < best_map_info.green_max; j++) {
				exact_defs[l].green = 65536 * j / best_map_info.green_max;
				for (k = 0; k < best_map_info.red_max; k++) {
					exact_defs[l].red = 65536 * k / best_map_info.red_max;
					l++;
				}
			}
		}
		...
		XSetStandardColormap(display, RootWindow(display, screen_num), &best_map_info, XA_RGB_BEST_MAP);
	}
