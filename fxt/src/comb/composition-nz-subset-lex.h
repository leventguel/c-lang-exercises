...
#define COMPOSITION_NZ_SUBSET_LEX_FIXARRAYS  // default on, speedup with prev()
...
public:
    ...
    virtual ~composition_nz_subset_lex()
    {
#ifndef COMPOSITION_NZ_SUBSET_LEX_FIXARRAYS
        delete [] a_;
#endif
    ...
    {
        const ulong z = a_[m_];
        if ( z<=1 )  // move one unit two places left:  [*, X, Y, 1] --> [*, X+1, Y]
        {
        ...
        else  // move all but one unit right:  [*, Y, Z] --> [*, Y, 1, Z-1]
        {
            a_[m_] = 1;
            ++m_;
            a_[m_] = z - 1;
    ...
    // Loopless algorithm.
    {
        ...
        if ( y==1 )  // add Z to left place: [*, 1, Z] --> [*, Z+1]
        {
            const ulong z = a_[m_];
            a_[m_-1] = z + 1;
            a_[m_] = 1;
        ...
        else  // move one unit two places right:  [*, Y, Z] --> [*, Y-1, Z, 1]
        {
            a_[m_-1] = y - 1;
            ++m_;
            // a[m] == 1 already
