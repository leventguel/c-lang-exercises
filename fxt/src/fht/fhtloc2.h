...
// Excellent performance for large array sizes.
{
    ...
    // Recursion:
//    for (ulong ldm=1; ldm<ldn; ++ldm)  fht_loc_dif2_core(f+(1UL<<ldm), ldm);
...
// Excellent performance for large array sizes.
{
    ...
    // Recursion:
//    for (ulong ldm=1; ldm<ldn; ++ldm)  fht_loc_dit2_core(f+(1UL<<ldm), ldm);
...
{
    // no special version for zero padded data yet:
    fht_loc_dif2_core(f, ldn);
    revbin_permute(f, 1UL<<ldn);
}
// -------------------------
