...
//    cout << "nsym=" << nsym << endl;
    ...
    string_subst strs(cmax, nsym, h3d_rules);
    jjassert( 0 == strs.verify(start) );

    ulong ctc = 0;
    for (ulong j=0; j<=ldn; ++j)  // show first generations of substitution
    {
    ...
    ulong t=0;  // position in string
//    while ( (0!=c[t]) && (c[t]!='F') )  ++t;  // skip leading rotations
